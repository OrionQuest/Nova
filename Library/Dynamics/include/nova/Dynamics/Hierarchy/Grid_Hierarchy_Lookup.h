...
class Grid_Hierarchy_Lookup
{
  ...
  public:
    ...
    {
        if(hierarchy.template Set<unsigned>(level,&Struct_type::flags).Is_Set(flags_offset,Cell_Type_Interior)) return true;
    ...
    static bool Cell_Lookup_Finer(const Hierarchy& hierarchy,uint64_t& flags_offset,int& level,TV& weights)
    {
        ...
        for(int candidate_level=level-1;candidate_level>=0;--candidate_level){
            ...
            if(hierarchy.template Set<unsigned>(candidate_level,&Struct_type::flags).Is_Set(candidate_flags_offset,Cell_Type_Interior)){
