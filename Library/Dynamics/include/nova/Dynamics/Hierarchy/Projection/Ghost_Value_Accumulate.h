//!#####################################################################
//! \file Ghost_Value_Accumulate.h
//!#####################################################################
// Class Ghost_Value_Accumulate
//######################################################################
...
class Ghost_Value_Accumulate
{
  ...
  public:
    Ghost_Value_Accumulate(Hierarchy& hierarchy,const std::pair<const uint64_t*,unsigned>& blocks,T Struct_type::* fine_data_channel,
                           T Struct_type::* coarse_data_channel,const int coarse_level)
    {
    ...
    void Run(Grid_Hierarchy<Struct_type,T,2>& hierarchy,const std::pair<const uint64_t*,unsigned>& blocks,
             T Struct_type::* fine_data_channel,T Struct_type::* coarse_data_channel,const int coarse_level) const
    {
        auto coarse_flags=hierarchy.Allocator(coarse_level).template Get_Const_Array<Struct_type,unsigned>(&Struct_type::flags);
        auto fine_data=hierarchy.Allocator(coarse_level-1).template Get_Const_Array<Struct_type,T>(fine_data_channel);
        auto coarse_data=hierarchy.Allocator(coarse_level).template Get_Array<Struct_type,T>(coarse_data_channel);

        auto ghost_value_accumulate=[&](uint64_t offset)
        {
        ...
        SPGrid_Computations::Run_Parallel_Blocks(blocks,ghost_value_accumulate);
    }
